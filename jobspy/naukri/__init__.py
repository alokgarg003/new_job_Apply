    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        while continue_search():
            ...
            try:
                ...
                if not job_details:
                    # debug: log the keys and a short snippet of the response to help backtracking
                    try:
                        keys = list(data.keys())
                    except Exception:
                        keys = None
                    log.warning(f"No job details found in API response. JSON keys: {keys}")
                    snippet = response.text[:800] if hasattr(response, "text") else None
                    if snippet:
