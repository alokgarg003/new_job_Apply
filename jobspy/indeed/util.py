...
def get_enum_from_job_type(label: str) -> JobType | None:
    """
    Map a job type label string to the corresponding JobType enum.
    Handles common variants by lowercasing and removing spaces/dashes.
    """
    label_clean = label.lower().replace("-", "").replace(" ", "")
    for jt in JobType:
        for variant in jt.value:
            if label_clean == variant.replace("-", "").replace(" ", "").lower():
                return jt
    return None
